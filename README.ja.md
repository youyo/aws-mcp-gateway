...
### パターン 1: 読み取り専用（Allow-list）

許可するアクションを明示的に列挙します。Deny-list 方式より安全（未列挙のアクションは暗黙的に拒否される）。

> **カバレッジの注意:** このリストは主要サービスをカバーしています。全 AWS サービスを網羅するには `arn:aws:iam::aws:policy/ReadOnlyAccess`（AWS 管理・自動更新）の利用を検討してください。ただし managed policy には `aws:CalledViaAWSMCP` 条件を付けられません。
...
> ⚠️ **このパターンはリモート実行権限を含みます（Read-Only ではありません）。**
