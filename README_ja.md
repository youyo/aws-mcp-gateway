...
3. Lambda ロールに `sts:AssumeRoleWithWebIdentity` 権限を追加

詳細なセットアップ手順（コマンド付き）: [Lambda デプロイ例 - federated モードとの組み合わせ](examples/lambda/README.md#federated-モードとの組み合わせロールチェーン)

> ⚠️ **AWS ロールチェーン制限**: 各ロールの `MaxSessionDuration` に関わらず、最大セッション時間は **1 時間** に制限されます。IDToken の有効期限（通常 1 時間）と合わせて、ユーザーは約 1 時間ごとに再認証が必要です。
